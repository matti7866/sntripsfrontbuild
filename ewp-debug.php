<?php
/**
 * Electronic Work Permit Information API - DEBUG VERSION
 */
...
$permitNumber = isset($_GET['permitNumber']) ? trim($_GET['permitNumber']) : '129398786';
...
curl_setopt($ch, CURLOPT_COOKIEJAR, dirname(__FILE__) . "/cookies_ewpi_debug.txt");
...
curl_setopt($ch, CURLOPT_COOKIEFILE, dirname(__FILE__) . '/cookies_ewpi_debug.txt');
$a = curl_exec($ch);
...
echo "<h3>Permit Number: $permitNumber</h3>";
...
// Output raw HTML (first 5000 chars)
echo "<h3>Raw HTML (first 5000 chars):</h3>";
echo "<pre>" . htmlspecialchars(substr($data, 0, 5000)) . "</pre>";
