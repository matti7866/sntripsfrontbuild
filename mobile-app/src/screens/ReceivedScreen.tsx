...
export default function ReceivedScreen() {
  ...
  const loadTasks = async () => {
    try {
      setLoading(true);
      const data = await eidService.getEIDTasks('received');
      setTasks(data.tasks || []);
  ...
  const handleMarkDelivered = async (task: ResidenceTask) => {
    Alert.alert(
      'Mark as Delivered',
      `Are you sure you want to mark ${task.passenger_name}'s Emirates ID as delivered?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await eidService.markEIDDelivered({
                id: task.residenceID,
                type: task.type || 'ML',
              });
              Alert.alert('Success', 'Emirates ID marked as delivered successfully');
              loadTasks();
            } catch (error: any) {
  ...
  const renderTask = ({ item }: { item: ResidenceTask }) => (
    <View style={styles.taskCard}>
      ...
      <TouchableOpacity style={styles.actionButton} onPress={() => handleMarkDelivered(item)}>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Received</Text>
        <Text style={styles.headerSubtitle}>
          {filteredTasks.length} of {tasks.length} task(s) ready for delivery
        </Text>
      </View>
